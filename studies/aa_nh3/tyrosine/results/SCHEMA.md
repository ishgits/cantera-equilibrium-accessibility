...
|---|---|---|---|
| study_id | str |  | Study identifier (folder name under studies/). |
| substudy_id | str |  | Substudy: inventory_landscape, deltaG_sweep, or nh3_deltaG_landscape. |
| target_product | str |  | Original target product, e.g. Alanine(aq). |
| target_variant | str |  | Cantera species actually modelled (base name or ΔG pseudo-species). |
| model_id | str |  | Hashed Cantera model identity (one YAML reused across grid points). |
...
| case_id | str |  | Unique simulation case identifier (canonical run key). |
...
| formation_call | str |  | significant \| trace \| below_threshold \| solver_failed. |
| solver_status | str |  | Equilibrium solver outcome: ok or failed. |
| error_message | str |  | Solver/setup error message when the case failed. |
...
| column | dtype | units | description |
|---|---|---|---|
| scenario | str |  | Scenario id carried from the base runner (equals case_id). |
| model_mode | str |  | Model mode tag, e.g. single_product_sensitivity. |
| yaml_file | str |  | Cantera YAML file name used for the case. |
| target_product | str |  | Original target product, e.g. Alanine(aq). |
| T_C | float64 | deg C | Equilibrium temperature. |
| T_K | float64 | K | Equilibrium temperature in Kelvin. |
| P_Pa | float64 | Pa | Equilibrium pressure. |
| species | str |  | Cantera species name for this row (long-form tables). |
...
| n_eq_mol | float64 | mol | Reconstructed equilibrium moles of the species. |
| element_balance_relative_spread | float64 |  | Relative spread of per-element total-mole estimates (QC). |
| element_total_mole_estimates | str |  | Per-element total-mole estimates (diagnostic string). |
